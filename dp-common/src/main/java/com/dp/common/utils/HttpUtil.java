...
public class HttpUtil {
		...
		try {
			...
			if (HttpEntityEnclosingRequestBase.class.isAssignableFrom(httpRequest.getClass())) {
				...
				if (url.contains("?") && (url.indexOf("?") < url.indexOf("="))) {
					// 如果url中存在参数，将url中的参数封装到NameValuePair
					map2NVPair(nvps, buildParams(url.substring(url.indexOf("?") + 1)));
				} else {
				...
				int idx = url.indexOf("?");
				log.info("请求地址：" + url.substring(0, (idx > 0 ? idx - 1 : url.length() - 1)));
				if (idx > 0) {
					log.info("请求参数：" + url.substring(idx + 1));
				}
			}
			// 调用发送请求
			log.info("开始请求: {}", url);
			body = execute(httpRequest, url, encoding, openSSL);
		} catch (Exception e) {
	...
	private static String execute(HttpRequestBase httpRequest, String url, String encoding, Boolean openSSL) {
		String body = "";
	 ...
	 * @Description 生成参数
	 */
	private static Map<String, Object> buildParams(String params) {
	 ...
	 * @throws Exception
	 */
	public static Map<String, Object> weiPayReq(String URL, HttpServletRequest request, List<Header> headers)
			throws Exception {
